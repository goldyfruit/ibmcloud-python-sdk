...
class VpnTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fake_vpn['default_security_group'] = 'fruit-average-shaping-gone-denture-rumor'
        self.fake_vpn['default_network_acl'] = 'unpainted-crucial-trimmer-perennial-zipfile-barcode'
...
##        response = self.vpn.get_vpn_default_security_group(self.fake_vpn['id'])
##        print(response)
##        self.assertEqual(response['default_security_group']['name'], self.fake_vpn['default_security_group'])
##
##    @patch('ibmcloud_python_sdk.vpc.vpn.qw', common.fake_get_one)
...
##        self.assertEqual(response['default_network_acl']['name'], self.fake_vpn['default_network_acl'])
##
