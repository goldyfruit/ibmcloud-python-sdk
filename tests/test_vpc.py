...
from . import vpc_test_config as tcfg

class VPCTestCase(unittest.TestCase):
    """Test case for the client methods."""

    def setUp(self):
        self.patcher = patch('ibmcloud_python_sdk.vpc.vpc.qw', tcfg.fake_get_call)
        self.patcher.start()
        self.vpc = Vpc()
        self.fake_vpc = {}
    ...
    @patch('ibmcloud_python_sdk.auth.get_token', tcfg.fake_auth)
...
#    @patch('ibmcloud_python_sdk.auth.get_token', tcfg.fake_auth)
#    @patch('ibmcloud_python_sdk.vpc.vpc.qw', tcfg.fake_get_one)
...
#        self.assertEqual(response['id'], self.fake_vpc['id'])
    ...
    @patch('ibmcloud_python_sdk.auth.get_token', tcfg.fake_auth)
    @patch('ibmcloud_python_sdk.vpc.vpc.qw', tcfg.fake_create_vpc)
