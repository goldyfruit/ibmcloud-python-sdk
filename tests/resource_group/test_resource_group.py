...
class ResourceGroupTestCase(TestCase):
    def setUp(self):
        ...
        self.content = get_one(self.type, 0)
        self.quota = self.get_quota(0)
    ...
    def test_get_resource_group_by_id_exception(self):
        with self.assertRaises(Exception):
            self.resource_group.get_resource_group_by_id(self.content['data']['id'])

    @patch('ibmcloud_python_sdk.resource.resource_group.qw', qw_exception)
    def test_get_resource_group_by_name_exception(self):
        with self.assertRaises(Exception):
            self.resource_group.get_resource_group_by_name(self.content['data']['name'])
